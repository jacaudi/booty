...
- **Verifies artifact integrity.** Downloads are checked before they land — Fedora CoreOS by
  SHA-256, Flatcar by detached GPG signature against an embedded key, Tails by the SHA-256 its
  netboot.xyz release publishes. The default policy (`warn`) refuses tampered (forged-signature)
  artifacts while logging other verification failures — except on a resumable multi-GB artifact,
  where a checksum failure is also refused; `--signaturePolicy strict` refuses all of them (see
